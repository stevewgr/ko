...
bool CN3ShapeMgr::Load(HANDLE hFile)
{
	...
	if(iSC > 0)
	{
		...
		for(int i = 0; i < iSC; i++)
		{
			...
			// pShape->m_iEventType; 0-바인드포인트 1-성문(좌우열림) 2-성문(상하열림) 3-레버(상하당김)
...
//			}
			...
			if(!(i%64))
			{
				...
				pUILoading->SetStringInfo(szBuff);
				pUILoading->SetValue(75 + 15 * i / iSC);
				pUILoading->Render();
			}
#endif // end of #ifndef _N3GAME
		}
...
void CN3ShapeMgr::GenerateCollisionData()
{
	...
	{
#ifdef _N3GAME
		CLogWriter::Write("CN3ShapeMgr::GenerateCollisionData - 충돌 체크 폴리곤의 점갯수와 면 갯수가 다릅니다. (%s)", m_szFileName.c_str());
#endif
		this->Release();
		return;
	}
...
//					}
					...
					{
#ifdef _N3GAME
						CLogWriter::Write("CN3ShapeMgr::GenerateCollisionData - 충돌 체크 폴리곤 수가 너무 많습니다. (%s)", m_szFileName.c_str());
#endif
						continue;
					}
